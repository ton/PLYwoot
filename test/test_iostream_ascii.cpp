...
TEST_CASE("Tests reading and writing vertex and face data", "[iostream][ascii]")
{
  ...
  using FaceLayout = plywoot::reflect::Layout<plywoot::reflect::Array<int, 3, char>>;
  const std::vector<Face> faces = plyFile.read<Face, FaceLayout>(faceElement);
  const std::vector<Face> expectedFaces{{0, 2, 1}, {0, 3, 2}, {4, 5, 6}, {4, 6, 7}, {0, 1, 5}, {0, 5, 4},
                                        {2, 3, 7}, {2, 7, 6}, {3, 0, 4}, {3, 4, 7}, {1, 2, 6}, {1, 6, 5}};
  REQUIRE(expectedFaces == faces);
